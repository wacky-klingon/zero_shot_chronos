...
class ChronosTrainer:
    ...
    def __init__(
        self,
        base_model_path: str = "data/models/base",
    ...
    def load_base_model(self) -> None:
        ...
        try:
            ...
            self.model = ChronosBoltPipeline.from_pretrained(
                str(self.base_model_path), torch_dtype=torch.float32, device_map="cpu"
            )
    ...
    def generate_dummy_data(self, n_samples: Optional[int] = None) -> np.ndarray:
        ...
        for i in range(n_samples):
            ...
            if "seasonal" in patterns:
                seasonal = 5 * np.sin(2 * np.pi * t / 24) + 2 * np.sin(
                    2 * np.pi * t / 7
                )
                series += seasonal
            ...
            with torch.no_grad():
                for i in range(0, len(train_contexts), 32):  # Batch size of 32
                    ...
                    predictions = self.model.predict(
                        batch_contexts, prediction_length=batch_targets.shape[1]
                    )
    ...
    def evaluate_model(
        ...
        metrics = {"mse": float(mse), "mae": float(mae), "avg_loss": float(avg_loss)}
